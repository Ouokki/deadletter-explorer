...
export async function getMessages(topic: string, limit = 200, opts: FetchOpts = {}): Promise<MessageDto[]> {
  const r = await fetch(`/api/dlq/messages?topic=${encodeURIComponent(topic)}&limit=${limit}`, { signal: opts.signal });
